...
- **解析离散伴随反向传播（Analytic Discrete-Adjoint Backward Pass）**：对于每个前向算子 $y = f(x)$，FlashPDE手动推导其离散伴随形式，即反向传播时梯度 $\bar{x}$ 的计算公式。例如，对于一维二阶导数算子 $y_i = (x_{i+1} - 2x_i + x_{i-1}) / h^2$，其反向传播公式为 $\bar{x}_i = (\bar{y}_{i-1} - 2\bar{y}_i + \bar{y}_{i+1}) / h^2$。这本质上是一个与正向算子相同形式的算子，因此可以复用前向内核，无需存储中间结果。
